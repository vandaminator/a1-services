...
function CartPage() {
    ...
    if (cart.length > 0) {
      ...
      if (dataNotLoaded) {
        const load = async () => {
          ...
          const response = await fetch(`/${storeId}/api/product/quantity`, {
            method: "PUT",
  ...
  }, [cart, changeItemsAvaliable, dataNotLoaded, storeId]);
  ...
  const handleBuy = () => {
    if (status === "authenticated") {
      const loading = async () => {
        ...
        const response = await fetch(`/${storeId}/api/order`, {
          method: "POST",
        ...
        if (response.ok) {
          ...
          removeAllItems();
          router.push("/${storeId}/cart/confirm");
        } else {
      ...
      signIn("credentials", {callbackUrl: `/${storeId}/cart`});
    }
  };
